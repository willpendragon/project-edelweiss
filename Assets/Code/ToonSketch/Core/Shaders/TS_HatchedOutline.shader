Shader "ToonSketch/Hatched Outlined"
{
    Properties
    {
		...
		[Toggle(_TS_HATCHING_ON)] _HatchShading("Hatch Shading?", Float) = 0
		...
		_HatchScale("Hatch Scale", Range(0, 30)) = 10
		_HatchThreshold("Hatch Threshold", Range(0, 1)) = 0.5
		_HatchStrength("Hatch Strength", Range(0, 1)) = 1
		[Enum(Texture,0,World XY,1,World XZ,2)] _HatchUVs("Hatch UVs", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			#pragma shader_feature _ _TS_HATCHING_ON
			...
			#ifdef _TS_HATCHING_ON
				#include "TS_Hatching.cginc"
			#endif
	...
	Fallback "Diffuse"
	CustomEditor "ToonSketchHatchedOutlineShaderGUI"
}
