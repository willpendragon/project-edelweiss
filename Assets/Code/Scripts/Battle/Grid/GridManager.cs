...
// --- NEW: Load Deity Shards (MUST BE BEFORE ENEMIES SO TILES ARE MARKED OCCUPIED) ---
        ...
        if (currentMapData.enemySpawnPositions != null && currentMapData.enemySpawnPositions.Count > 0)
        {
            foreach (var spawnData in currentMapData.enemySpawnPositions)
            {
                ...
                // OCCUPANCY CHECK: Skip if tile is already occupied by shard or other unit
                if (targetTile != null && targetTile.currentSingleTileCondition == SingleTileCondition.free)
                {
        ...
        // Deity Shards moved to BEFORE enemy spawning (see above)
...
// Usiamo questa per cercare i tile al "Piano Terra" (Elevazione 0).
    public TileController GetTileControllerInstance(int xCoordinate, int zOrOldYCoordinate)
    {
        ...
        // finch� non troviamo il primo blocco fisico esistente.
        for (int y = 20; y >= 0; y--)
        {
    ...
    /// <summary>
    /// Posiziona un GameObject (es. Unit�) esattamente sopra la superficie calpestabile di un Tile,
    ...
    public void PlaceUnitOnTileSurface(GameObject unitToPlace, TileController targetTile)
    {
        ...
        // Anzich� usare posizioni relative o bounds, usiamo la cima esatta del Renderer o del Collider!
        float finalY = targetTile.transform.position.y;

        Collider col = targetTile.GetComponent<Collider>();
        if (col != null)
        {
            // bounds.max.y ti d� il punto ASSOLUTO in altezza top nello spazio mondo di QUEL cubo!
