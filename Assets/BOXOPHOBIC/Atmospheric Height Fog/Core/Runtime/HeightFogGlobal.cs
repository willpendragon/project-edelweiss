...
namespace AtmosphericHeightFog
{
    ...
    public class HeightFogGlobal : StyledMonoBehaviour
    {
        ...
        public static HeightFogGlobal Instance;
        ...
        [Space(10)]
        public FogTriggerMode overrideTriggerMode = FogTriggerMode.AlwaysUseTriggerPoint;
        public Transform overrideTriggerPoint;

        [Space(10)]
        public bool autoAssingSceneObjects = true;
        //public bool autoFogPositionAndScale = true;
        ...
        Material overrideMaterial0;
        Material overrideMaterial1;

        MeshRenderer meshRenderer;

        [System.NonSerialized]
        public List<HeightFogOverride> heightFogOverrides = new List<HeightFogOverride>();
        ...
        void OnEnable()
        {
            ...
            heightFogOverrides = new List<HeightFogOverride>();

            //if (!autoFogPositionAndScale)
            {
                gameObject.transform.position = Vector3.zero;
                gameObject.transform.rotation = Quaternion.identity;
                gameObject.transform.localScale = new Vector3(100000, 100000, 100000);
            }

            if (autoAssingSceneObjects)
            {
                GetCamera();
                GetDirectional();
            ...
            if (mainCamera != null)
            {
                EnableCameraDepthInBIRP(mainCamera);
            }
            ...
            localMaterial.name = "Height Fog Local";

            blendMaterial = new Material(localMaterial);
            blendMaterial.name = "Height Fog Blend";
        ...
        void Update()
        {
            ...
            var overrideLerp = 0.0f;

            overrideMaterial0 = null;
            overrideMaterial1 = null;

            for (int i = 0; i < heightFogOverrides.Count; i++)
            {
                var overrideObject = heightFogOverrides[i];
                ...
                if (!overrideObject.isActive)
                {
                    continue;
                }

                if (overrideObject.overrideDistance > 0)
                {
                    if (overrideMaterial0 == null)
                    {
                        overrideMaterial0 = overrideObject.currentMaterial;

                        overrideLerp = overrideObject.overrideDistance;
                    }
                    else
                    {
                        overrideMaterial1 = overrideObject.currentMaterial;
                    }
                }
            }
            ...
            if (overrideLerp > 0)
            {
                if (overrideMaterial0 != null)
                {
                    if (overrideLerp == 1)
                    {
                        blendMaterial = overrideMaterial0;
                    }
                    else
                    {
                        if (overrideMaterial1 != null)
                        {
                            blendMaterial.Lerp(overrideMaterial1, overrideMaterial0, overrideLerp);
                        }
                        else
                        {
                            blendMaterial.Lerp(currentMaterial, overrideMaterial0, overrideLerp);
                        }
                    }
                }
            }
        ...
        void EnableManager()
        {
            if (HeightFogGlobal.Instance != null && HeightFogGlobal.Instance != this)
            {
                HeightFogGlobal.Instance.gameObject.SetActive(false);
            }

            Instance = this;
            Instance.name = "Atmospheric Height Fog";
        ...
        {
            if (overrideTriggerPoint == null)
            {
                if (mainCamera != null)
                {
                    overrideTriggerPoint = mainCamera.transform;
                }
            }
        }
        ...
        public void EnableCameraDepthInBIRP(Camera camera)
        {
            if (camera.depthTextureMode != DepthTextureMode.Depth || camera.depthTextureMode != DepthTextureMode.DepthNormals)
            {
                camera.depthTextureMode = DepthTextureMode.Depth;
            }
        }

        void SetFogSphereTransform()
        {
